...
Control_Flow_IC_Stmt::Control_Flow_IC_Stmt(Tgt_Op inst_op, Ics_Opd * o1, Ics_Opd * o2, string lbl)
{
	op_desc = *(machine_desc_object.spim_instruction_table[inst_op]);
	offset = lbl;
	opd1 = o1;
...
void Control_Flow_IC_Stmt::set_Offset(string io) { offset = io ;}
...
Label_IC_Stmt::Label_IC_Stmt(Tgt_Op op, Ics_Opd * o1, string res)
{
	CHECK_INVARIANT((machine_desc_object.spim_instruction_table[op] != NULL),
			"Instruction description in spim table cannot be null");
...
void Label_IC_Stmt::set_offset(string io) { offset = io; }
